...
module Jekyll
  module WordPress
    ...
    #                   Default: true.
    # :encoding::       If specified, strings will be outputed
    ...
    def self.process_post(post, db, options)
      ...
      name = "%02d-%02d-%02d-%s.html" % [date.year, date.month,
                                             date.day, slug]
      ...
      if options[:comments] and post[:comment_count].to_i > 0
        ...
        db[cquery].each do |comment|
          ...
          comments << {
            'id'           => comment[:id].to_i,
            'author'       => comauthor,
            'author_email' => comment[:author_email].to_s.force_encoding('UTF-8'),
            'author_url'   => comment[:author_url].to_s.force_encoding('UTF-8'),
            'date'         => comment[:date].to_s.force_encoding('UTF-8'),
            'date_gmt'     => comment[:date_gmt].to_s.force_encoding('UTF-8'),
            'content'      => comcontent,
          }
        end
      ...
      data = {
        'layout'        => post[:type].to_s.force_encoding('UTF-8'),
        'status'        => post[:status].to_s.force_encoding('UTF-8'),
        'published'     => (post[:status].to_s == "publish"),
        'title'         => title.to_s,
        'author'        => post[:author].to_s.force_encoding('UTF-8'),
        'author_login'  => post[:author_login].to_s.force_encoding('UTF-8'),
        'author_email'  => post[:author_email].to_s.force_encoding('UTF-8'),
        'author_url'    => post[:author_url].to_s.force_encoding('UTF-8'),
        'excerpt'       => excerpt,
    ...
    def self.add_ps(text)
      text = "<div class='wp_imported'><p/>#{text}</div>"
      text.gsub!(/\n/, "\n<p/>")
    end
